    ...
    def __init__(self, sentence_pack, args):
        ...
        for triple in sentence_pack['triples']:
            ...
            for i, object in enumerate(objects):
                # Since there is only one aspect in each triples of the data set, and there may be multiple opinions, they need to be constructed separately
    ...
    for ex_index, example in enumerate(train_instances):
        ...
        for triple_name in sample['triples']:
            ...
            if aspect_span not in aspect:
                ...
                print(aspect[aspect_span],sentiment,sample['triples'])
        ...
        if differ_opinion_sentiment:
            ...
            print(ex_index, 'Single Opinion Word Multipolarity')
            continue
        ...
        #     print('There are different triples using repeated aspect:', example.id)
...
def load_data(args, path, if_train=False):
    ...
    print(os.getcwd())
    #path='/mnt/SBN/datasets/SemEval-Triplet-data/ASTE-Data-V2-EMNLP2020/14lap/test_triplets.txt'
    #os.path
    ...
    for ex_index, line in enumerate(lines):
        ...
        for triple in raw_pairs:
            ...
            if word not in triple_dict:
                ...
                print('single sentence' + id + 'repeated triplet！')
...
class DataTterator(object):
    def __init__(self, instances, args):
        ...
        #self.tokenizer = BertForMaskedLM.from_pretrained(args.init_vocab)
        #self.tokenizer = BertTokenizer.from_pretrained('/mnt/SBN/BioBERT')
    ...
    def get_batch(self, batch_num):
        ...
        for i, sample in enumerate(self.instances[batch_num]):
            logger.info('Sample in enumerate:{}'.format(sample))
            ...
            #The maximum number of tokens and spans is set to a fixed value
        ...
        '''Since the size of the square matrix is ​​different in different sentences, it is not converted to tensor'''
        ...
        for tokens_tensor, bert_spans_tensor, spans_ner_label_tensor, spans_aspect_tensor, spans_opinion_label_tensor, \
            ...
            if spans_pad_length > 0:
                ...
                '''Similar to square matrix mask for span'''
            ...
            if final_tokens_tensor is None:
                ...
                if final_tokens_tensor.size(-1) == tokens_tensor.size(-1):
                    ...
                    logger.info("get_batch | final_tokens_tensor{}tokens_tensor{}".format(final_tokens_tensor.shape,tokens_tensor.shape))

        ## Note that the maximum span interval in the feature is not necessarily the set max_span_length, this is because the span caused by bert segmentation is expanded.
