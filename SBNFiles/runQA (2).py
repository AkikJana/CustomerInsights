...
from transformers import AdamW, BertModel, get_linear_schedule_with_warmup, BertForMaskedLM
from models.data_BIO_loader import load_data, DataTterator
# from models.SemEval import load_data
# from models.SemEval import DataTterator
...
def eval(bert_model, step_1_model, step_2_forward, step_2_reverse, dataset, args):
    with torch.no_grad():
        ...
        for j in range(dataset.batch_count):
            ...
            bert_features = bert_model(input_ids=tokens_tensor, attention_mask=attention_mask, output_hidden_states=True)
            ...
            if torch.nonzero(pred_aspect_logits, as_tuple=False).shape[0] == 0:
                ...
                #     bert_features.hidden_states[len(bert_features.hidden_states)-1], attention_mask, bert_spans_tensor, spans_mask_tensor,
            ...
            if torch.nonzero(reverse_pred_stage1_logits, as_tuple=False).shape[0] == 0:
                ...
                # reverse_spans_embedding, reverse_span_mask = stage_2_features_generation(bert_features.hidden_states[len(bert_features.hidden_states)-1],
                #                                                                          attention_mask,
                #                                                                          bert_spans_tensor,
                #                                                                          spans_mask_tensor,
                #                                                                          spans_embedding,
...
def train(args):
    ...
    logger.info('test_dataset:{}'.format(test_datasets))
    testset = DataTterator(test_datasets, args)
    print('测试集加载完成')
    logger.info('测试集加载完成')
    print('-------------------------------')
    print(args)
    #/mnt/SBN/savemodel/BioBERTweet
    Bert = BertModel.from_pretrained(args.init_model)
    #Bert = BertForMaskedLM.from_pretrained(args.init_model)
    #Bert = BertForMaskedLM.from_pretrained('/mnt/SBN/savemodel/BioBERTweet')
    #Bert = BertModel.from_pretrained('/mnt/SBN/BioBERT')
    bert_config = Bert.config
    print(Bert.config)
    logger.info(Bert.config)
    Bert.to(args.device)
    ...
    if args.mode == 'train':
        ...
        logger.info('train_dataset:{}'.format(train_datasets))
        ...
        logger.info('dev dataset: {}'.format(dev_datasets))
        ...
        for i in range(args.epochs):
            ...
            # for j in range(trainset.batch_count):
                ...
                #logger.info(trainset.get_batch(j))
                tokens_tensor, attention_mask, bert_spans_tensor, spans_mask_tensor, spans_ner_label_tensor, \
                spans_aspect_tensor, spans_opinion_label_tensor, reverse_ner_label_tensor, reverse_opinion_tensor, \
                reverse_aspect_label_tensor, related_spans_tensor, sentence_length = trainset.get_batch(j)
                #print(tokens_tensor,len(tokens_tensor),attention_mask,len(attention_mask))
                bert_output = Bert(input_ids=tokens_tensor, attention_mask=attention_mask, output_hidden_states=True)
                ##print(bert_output)
                #logger.info("Hidden States=>",bert_output.hidden_states[0],len(bert_output.hidden_states[0]))
                #logger.info("Last hidden?=>",bert_output.last_hidden_state)
                aspect_class_logits, opinion_class_logits, spans_embedding, forward_embedding, reverse_embedding, \
                cnn_spans_mask_tensor = step_1_model(
            ...
            if triplet_result[2] >= best_triple_f1 or triplet_result[2] >= 0.55:
    ...
    #model_path = '/mnt/SBN/savemodel/BioBERTweet/pytorch_model.bin'
    if args.muti_gpu:
...
def main():
    ...
    parser.add_argument('--model_dir', type=str, default="./BioBERT/", help='model path prefix')
    parser.add_argument('--device', type=str, default="cpu", help='cuda or cpu')
    parser.add_argument("--init_model", default="dmis-lab/biobert-large-cased-v1.1-squad", type=str, required=False,help="Initial model.")
    #parser.add_argument("--init_vocab", default="/mnt/SBN/BioBERT", type=str, required=False,help="Initial vocab.")
    #parser.add_argument("--init_model", default="/mnt/SBN/savemodel/BioBERTweet", type=str, required=False,help="Initial model.")
    parser.add_argument("--init_vocab", default="dmis-lab/biobert-large-cased-v1.1-squad", type=str, required=False,help="Initial vocab.")


    parser.add_argument("--bert_feature_dim", default=1024, type=int, help="feature dim for bert")
    ...
    parser.add_argument('--epochs', type=int, default=5, help='training epoch number')
    parser.add_argument("--train_batch_size", default=16, type=int, help="batch size for training")
    parser.add_argument("--RANDOM_SEED", type=int, default=2023, help="")
    '''修改了数据格式'''
    parser.add_argument("--dataset_path", default="/mnt/SBN/datasets/CITwitterTripletData/",
                        choices=["./datasets/BIO_form/", "/mnt/SBN/datasets/SemEval-Triplet-data/ASTE-Data-V2-EMNLP2020", "/mnt/SBN/datasets/CITwitterTripletData"],
                        help="")
    parser.add_argument("--dataset", default="Bio1", type=str, choices=["Bio1", "14lap", "res14", "res15", "res16"],
