...
def stage_2_features_generation(bert_feature, attention_mask, spans, span_mask, spans_embedding, spans_aspect_tensor,
    ...
    # Process the input aspect information and remove invalid aspect span
    ...
    for i, spans_aspect_tensor_unspilt in enumerate(spans_aspect_tensor_spilt):
        ...
        if spans.size(0)!=batch_num-1:
            logger.info("Issue related to batch number batch_num{} and spans{}".format(batch_num,spans))
        # batch_num=batch_num-1
        # if batch_num!=spans.size(0)-1:
        #     batch_num=batch_num-1
        while batch_num>=spans.size(0):
            batch_num=batch_num-1
        ...
        if len(spans_embedding[batch_num, span_index, :].unsqueeze(0).size())==4:
            # logger.info("dimension size ")
            # aspect_span_embedding_unspilt = spans_embedding[batch_num, span_index, :]
            aspect_span_embedding_unspilt = spans_embedding[batch_num, span_index, :]
            #continue
        else:    
        ...
        if all_span_aspect_tensor is None:
            ...
            #all_span_aspect_tensor=all_span_aspect_tensor
            #logger.log(all_span_aspect_tensor.size(),aspect_span_embedding_unspilt.size(),all_bert_embedding.size(),bert_feature_unspilt.size(),all_attention_mask.size(),attention_mask_unspilt.size(),all_spans_embedding.size(),spans_embedding_unspilt.size())
            logger.info('all_span_aspect_tensor:=> {}\taspect_span_embedding_unspilt:=> {}'.format(all_span_aspect_tensor.size(),aspect_span_embedding_unspilt.size()))
            #torch.cat((x[..., None], y), -1)
            if spans_opinion_tensor is not None:
                all_span_opinion_tensor = torch.cat((all_span_opinion_tensor, spans_opinion_tensor_unspilt), dim=0)
            #logger.info(all_span_aspect_tensor.size(),aspect_span_embedding_unspilt.size(),all_bert_embedding.size(),bert_feature_unspilt.size(),all_attention_mask.size(),attention_mask_unspilt.size(),all_spans_embedding.size(),spans_embedding_unspilt.size())
...
class Pointer_Block(torch.nn.Module):
    ...
    def forward(self, hidden_embedding, masks, encoder_embedding):
        #Note that the mask needs to match the scores in the attention to remove the corresponding meaningless values
