...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        flag = True
        for i,j in zip(query_layer.size()[:-2],key_layer.transpose(-1, -2).size()[:-2]):
            if i!=j:
                logger.info("mismatching tensors for matrix multiplication")
                flag=False
        if query_layer.size(-1) == key_layer.transpose(-1, -2).size(-2) and flag:
            ...
            logger.info("Matrix multiplication not possible.query_layer:{}key_layer:{}".format(query_layer.size(),key_layer.transpose(-1, -2).size()))
